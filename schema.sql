-- ============================================
-- TechSupport Pro - Supabase Schema
...
-- Statuses
CREATE POLICY "authenticated_read_statuses"
  ON statuses FOR SELECT
  USING (auth.role() = 'authenticated');

CREATE POLICY "admin_write_statuses"
  ON statuses FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role = 'admin'));

CREATE POLICY "admin_delete_statuses"
...
CREATE POLICY "write_own_or_assigned_tickets"
...
CREATE POLICY "user_read_own_notif_prefs"
  ON notification_preferences FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "user_upsert_own_notif_prefs"
  ON notification_preferences FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "user_update_own_notif_prefs"
  ON notification_preferences FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- Appearance Preferences
CREATE POLICY "user_read_own_appearance"
  ON appearance_preferences FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "user_upsert_own_appearance"
  ON appearance_preferences FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "user_update_own_appearance"
